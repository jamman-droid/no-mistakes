...
- Declines to resume a parked run whose agent role policy no longer resolves to the snapshot recorded when it started, so an `agent`/`agent_roles` edit made while it was parked cannot silently re-attribute its remaining work to a different candidate; runs started before that snapshot existed are unaffected
