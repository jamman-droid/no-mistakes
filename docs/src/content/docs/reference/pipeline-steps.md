...
- Runs each reviewer turn as a fresh, session-free invocation using the `agent_roles.reviewer` selection; all other work, including review fixes, uses `agent_roles.implementer` (both inherit `agent` when omitted)
- Round 1 reviews the whole relevant change slice; after the implementer independently evaluates the findings and applies only the ones it agrees with, round 2 verifies only that agreed-fix diff rather than repeating the whole-slice review
- Allows reviewer round 3 only when round 2 explicitly surfaces a newly introduced or exposed severe correctness or security defect; round 3 is terminal, so the implementer may apply its returned fixes but no confirmation pass or round 4 runs
- Treats implementer disagreement, infrastructure failures, protective aborts, and correction of a false claim as non-review work; none resets or bypasses the three-review cap
- Reframes fix-round changes as pipeline-authored code within the narrowed fix scope, with prior findings, fix summaries, and same-round tests treated as claims rather than evidence
- With the default `session_reuse: true`, Claude and Codex reuse one durable implementer/fixer session across review-fix turns; a resume failure retries the same fix turn in a fresh implementer session, and unsupported agents run cold
- Atomically records the exact commit examined when the review loop completes successfully; a parked review retains its candidate only for recovery, while failed, skipped, superseded, and legacy reviews grant no inferred approval authority
...
**Auto-fix:** the implementer receives the selected previous findings plus any per-finding user notes, any selected user-authored findings from the TUI or AXI interface, and a sanitized history of prior rounds for that step, including earlier fix summaries and which findings the user left unselected.
It evaluates each finding from the code and evidence without a preferred outcome: applying a legitimate fix and declining an unsupported finding are equally acceptable. It applies all agreed fixes before running one focused verification limited to the changed area, and it is instructed not to run the complete repository test or lint suite during the fix round.
