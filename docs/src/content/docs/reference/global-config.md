...
`provider` and `model` declare identity; they do not synthesize flags because provider/model flag syntax differs by harness. When present, each must be non-empty, at most 128 ASCII characters with no surrounding whitespace, and contain only letters, digits, `.`, `_`, `-`, `/`, `:`, `+`, or `@`. The `:` and `@` version separators real model IDs use (`llama3:8b`, `claude-3-7@20250219`) are accepted, but shapes that would smuggle a URL, a credential, or a path into identity are rejected: a scheme or drive separator (`:/`), userinfo credentials (an `@` preceded by a `:`), a backslash, a leading `/`, and empty, `.`, or `..` path segments. They are intentionally observable, so do not put credentials in these fields. Invocation records keep adapter-reported provider/model values when available and use the declarations when the adapter does not report them.

Evidence surfaces such as `no-mistakes roles resolve` and `no-mistakes axi attest` apply a narrower vocabulary than this parse rule: an accepted identity they cannot represent literally, including anything containing `:` or `@`, is emitted as a stable `redacted-<fingerprint>` token that keeps distinct candidates distinguishable without reprinting the raw value.
